...
def make_fig2_combined() -> None:
    ...
    df_sig_pb.to_csv(RESULTS_DIR / "Grant_PB_CXCR3pos_zinc_signature_per_cell.csv", index=False)
    df_sig_csf.to_csv(RESULTS_DIR / "Grant_CSF_CXCR3pos_zinc_signature_per_cell.csv", index=False)
    ...
    pd.DataFrame([{k: v for k, v in sig_test_pb.items() if k != "per_patient_values"}]).to_csv(
        RESULTS_DIR / "Grant_PB_CXCR3pos_zinc_signature_patient_level_test.csv", index=False
    )
    pd.DataFrame([{k: v for k, v in sig_test_csf.items() if k != "per_patient_values"}]).to_csv(
        RESULTS_DIR / "Grant_CSF_CXCR3pos_zinc_signature_patient_level_test.csv", index=False
    )
    ...
    robust_csf["per_patient_df"].to_csv(
        RESULTS_DIR / "Grant_CSF_CXCR3pos_signature_leave_one_out.csv", index=False
    )
    robust_pb["per_patient_df"].to_csv(
        RESULTS_DIR / "Grant_PB_CXCR3pos_signature_leave_one_out.csv", index=False
    )
    pd.DataFrame([
        {k: v for k, v in r.items() if k != "per_patient_df"}
        for r in (robust_csf, robust_pb)
    ]).to_csv(RESULTS_DIR / "Grant_signature_outlier_robustness_summary.csv", index=False)
    ...
    confound_pb["per_dataset"].to_csv(
        RESULTS_DIR / "Grant_PB_CXCR3pos_dataset_confounding_check.csv", index=False
    )
    confound_csf["per_dataset"].to_csv(
        RESULTS_DIR / "Grant_CSF_CXCR3pos_dataset_confounding_check.csv", index=False
    )
    pd.DataFrame([
    ...
    ]).to_csv(RESULTS_DIR / "Grant_dataset_confounding_summary.csv", index=False)
    ...
    df_sig_pb_unsigned.to_csv(RESULTS_DIR / "Grant_PB_CXCR3pos_zinc_signature_UNSIGNED_per_cell.csv", index=False)
    df_sig_csf_unsigned.to_csv(RESULTS_DIR / "Grant_CSF_CXCR3pos_zinc_signature_UNSIGNED_per_cell.csv", index=False)
    ...
    pd.DataFrame([{k: v for k, v in sig_test_pb_unsigned.items() if k != "per_patient_values"}]).to_csv(
        RESULTS_DIR / "Grant_PB_CXCR3pos_zinc_signature_UNSIGNED_patient_level_test.csv", index=False
    )
    pd.DataFrame([{k: v for k, v in sig_test_csf_unsigned.items() if k != "per_patient_values"}]).to_csv(
        RESULTS_DIR / "Grant_CSF_CXCR3pos_zinc_signature_UNSIGNED_patient_level_test.csv", index=False
    )
    print(
    ...
    stem = "Grant_CXCR3pos_PB_CSF_MS_vs_HC_4panel"
    ...
    _save_single_volcano(
        ...
        out_png=supp_dir / "Grant_supp_PB_volcano.png",
        out_pdf=supp_dir / "Grant_supp_PB_volcano.pdf",
    )
    _save_single_volcano(
        df_csf, label="B", zinc_color=SCATTER_COLOR_ZINC_CSF,
        out_png=supp_dir / "Grant_supp_CSF_volcano.png",
        out_pdf=supp_dir / "Grant_supp_CSF_volcano.pdf",
    )
    ...
    fig_supp_sig.savefig(supp_dir / "Grant_supp_unsigned_signature.png", dpi=300, bbox_inches="tight")
    fig_supp_sig.savefig(supp_dir / "Grant_supp_unsigned_signature.pdf", bbox_inches="tight")
    plt.close(fig_supp_sig)
 
    df_pb.to_csv(out_dir / "Grant_PB_CXCR3pos_MS_vs_HC_full_DE.csv", index=False)
    df_csf.to_csv(out_dir / "Grant_CSF_CXCR3pos_MS_vs_HC_full_DE.csv", index=False)
    pd.DataFrame({"gene": pb_bar_genes}).to_csv(out_dir / "Grant_PB_CXCR3pos_zinc_bar_genes.csv", index=False)
    pd.DataFrame({"gene": csf_bar_genes}).to_csv(out_dir / "Grant_CSF_CXCR3pos_zinc_bar_genes.csv", index=False)
