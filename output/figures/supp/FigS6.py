...
"""
FigS6.py — CXCR3+ vs CXCR3- comparison within PB, Supplementary Figure 6
...
TISSUE = "PB"  # the only thing that differs from FigS7.py (which uses "CSF")
...
def make_figS6_cxcr3_PB() -> None:
    ...
    out_dir = FIG_DIRS.get("supp_cxcr3_paired_pb", RESULTS_DIR / "supp" / "cxcr3_paired_pb")
    ...
    if "HC" in df_sig["condition"].unique():
        df_sig[df_sig["condition"] == "HC"].to_csv(
            out_dir / f"FigS6_CXCR3_{TISSUE}_HC_zinc_signature_per_cell.csv",
            index=False
        )
    if "MS" in df_sig["condition"].unique():
        df_sig[df_sig["condition"] == "MS"].to_csv(
            out_dir / f"FigS6_CXCR3_{TISSUE}_MS_zinc_signature_per_cell.csv",
            index=False
        )
    ...
    for name, res in [("HC", sig_test_hc), ("MS", sig_test_ms)]:
        pd.DataFrame([{k: v for k, v in res.items() if k != "per_patient_values"}]).to_csv(
            out_dir / f"FigS6_CXCR3_{TISSUE}_{name}_zinc_signature_patient_level_test.csv",
            index=False,
        )
    ...
    stem = f"FigS6_CXCR3_{TISSUE}_6panel"
