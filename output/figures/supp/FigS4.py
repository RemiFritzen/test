...
"""
figS4.py — PB + CSF CXCR3- MS vs HC zinc signature (Supplementary Figure 4)

This is the CXCR3- counterpart to fig2.py's CXCR3+ MS-vs-HC comparison --
same statistical framework, applied to the CXCR3- ("bystander") B cell
population instead, as a check on whether the disease effect seen in
CXCR3+ B cells is specific to that population or also present more broadly.
...
STATISTICS (identical framework to fig2.py -- see that file for full
rationale): patient-level pseudobulk throughout, to avoid pseudoreplication
(per-cell tests inflate n from n_patients to n_cells). HC vs MS is an
UNPAIRED, between-patient comparison (each patient belongs to only one
condition), using `pseudobulk_de` / `pseudobulk_signature_test` from
stats_utils.py -- NOT the paired form used for CXCR3+/- or PB/CSF
comparisons, where the same patient contributes both arms.

The per-cell scatter/violin plot itself is left as-is (a valid and
informative visualization of the cell-level distribution) -- only the
p-value annotation and gene-level statistics come from the patient-level
test.
"""
...
def compute_cell_signature(adata, zinc_genes: list[str], tissue: str, gene_signs: dict[str, float] | None = None) -> pd.DataFrame:
    ...
    equally justified for every comparison (see draw_violin/make_figS4_hc_vs_ms_cxcr3neg).
...
def make_figS4_hc_vs_ms_cxcr3neg() -> None:
    ...
    out_dir = FIG_DIRS.get("supp_cxcr3neg_hc_vs_ms", RESULTS_DIR / "supp" / "cxcr3neg_hc_vs_ms")
    ...
    df_sig_pb.to_csv(out_dir / "FigS4_PB_CXCR3neg_zinc_signature_per_cell.csv", index=False)
    df_sig_csf.to_csv(out_dir / "FigS4_CSF_CXCR3neg_zinc_signature_per_cell.csv", index=False)
    ...
    pd.DataFrame([{k: v for k, v in sig_test_pb.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS4_PB_CXCR3neg_zinc_signature_patient_level_test.csv", index=False
    )
    pd.DataFrame([{k: v for k, v in sig_test_csf.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS4_CSF_CXCR3neg_zinc_signature_patient_level_test.csv", index=False
    )
    ...
    confound_pb["per_dataset"].to_csv(
        out_dir / "FigS4_PB_CXCR3neg_dataset_confounding_check.csv", index=False
    )
    confound_csf["per_dataset"].to_csv(
        out_dir / "FigS4_CSF_CXCR3neg_dataset_confounding_check.csv", index=False
    )
    pd.DataFrame([
    ...
    ]).to_csv(out_dir / "FigS4_dataset_confounding_summary.csv", index=False)

    # Unsigned (plain-mean) version of the same signature, computed
    # alongside the sign-corrected one for transparency -- see
    # make_figS4_hc_vs_ms_cxcr3neg's supplementary figure below and the discussion of
    ...
    df_sig_pb_unsigned.to_csv(out_dir / "FigS4_PB_CXCR3neg_zinc_signature_UNSIGNED_per_cell.csv", index=False)
    df_sig_csf_unsigned.to_csv(out_dir / "FigS4_CSF_CXCR3neg_zinc_signature_UNSIGNED_per_cell.csv", index=False)
    ...
    pd.DataFrame([{k: v for k, v in sig_test_pb_unsigned.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS4_PB_CXCR3neg_zinc_signature_UNSIGNED_patient_level_test.csv", index=False
    )
    pd.DataFrame([{k: v for k, v in sig_test_csf_unsigned.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS4_CSF_CXCR3neg_zinc_signature_UNSIGNED_patient_level_test.csv", index=False
    )
    print(
    ...
    stem = "FigS4_CXCR3neg_PB_CSF_MS_vs_HC_4panel"
    ...
    _save_single_volcano(
        ...
        out_png=supp_dir / "FigS4_supp_PB_volcano.png",
        out_pdf=supp_dir / "FigS4_supp_PB_volcano.pdf",
    )
    _save_single_volcano(
        df_csf,
        ...
        out_png=supp_dir / "FigS4_supp_CSF_volcano.png",
        out_pdf=supp_dir / "FigS4_supp_CSF_volcano.pdf",
    )
    ...
    fig_supp_sig.savefig(supp_dir / "FigS4_supp_unsigned_signature_violin.png", dpi=300, bbox_inches="tight")
    fig_supp_sig.savefig(supp_dir / "FigS4_supp_unsigned_signature_violin.pdf", bbox_inches="tight")
    plt.close(fig_supp_sig)

    df_pb.to_csv(out_dir / "FigS4_PB_CXCR3neg_MS_vs_HC_full_DE.csv", index=False)
    df_csf.to_csv(out_dir / "FigS4_CSF_CXCR3neg_MS_vs_HC_full_DE.csv", index=False)
    pd.DataFrame({"gene": pb_bar_genes}).to_csv(out_dir / "FigS4_PB_CXCR3neg_zinc_bar_genes.csv", index=False)
    pd.DataFrame({"gene": csf_bar_genes}).to_csv(out_dir / "FigS4_CSF_CXCR3neg_zinc_bar_genes.csv", index=False)
